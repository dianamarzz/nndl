        ...
        reader.onload = (e) => {
            try {
                ...
                return tf.tidy(() => {
                    ...
                    // Create pixel tensor and normalize to [0,1]
                    const pixelsTensor = tf.tensor2d(pixels, [labels.length, 784]);
                    // Normalize to [0,1] - CRITICAL for neural network training
                    const normalized = pixelsTensor.div(255.0);
                    
                    // Reshape to [N, 28, 28, 1] for CNN input
                    const xs = normalized.reshape([labels.length, 28, 28, 1]);
                    
                    // Return cloned tensors (they'll be detached from tf.tidy)
                    return {
                        xs: tf.clone(xs),
                        ys: tf.clone(ys)
                    };
...
function splitTrainVal(xs, ys, valRatio = 0.1) {
    return tf.tidy(() => {
        ...
        // Split indices
        const trainIndices = tf.tensor1d(indices.slice(0, trainSize), 'int32');
        const valIndices = tf.tensor1d(indices.slice(trainSize), 'int32');
...
function draw28x28ToCanvas(imgTensor, canvas, scale = 4) {
    const tensor = imgTensor.squeeze();
