...
function linesToTensors(lines) {
    return tf.tidy(() => {
        ...
        const ys = tf.oneHot(labelsTensor, 10).toFloat(); // Convert to float32 for compatibility
...
export function splitTrainVal(xs, ys, valRatio = 0.1) {
    return tf.tidy(() => {
        ...
        // Create random permutation of indices as Uint32Array
        const indicesArray = tf.util.createShuffledIndices(numSamples);
        
        // Split the indices array into train and validation
        // Convert Uint32Array to regular array and then slice
        const trainIndicesArray = Array.from(indicesArray.slice(0, trainSize));
        const valIndicesArray = Array.from(indicesArray.slice(trainSize));
        
        // Convert arrays to tensors
        const trainIndices = tf.tensor1d(trainIndicesArray, 'int32');
        const valIndices = tf.tensor1d(valIndicesArray, 'int32');

        // Gather the data using the index tensors
...
export function getRandomTestBatch(xs, ys, k = 5) {
    return tf.tidy(() => {
        ...
        const indicesArray = [];
        for (let i = 0; i < k; i++) {
            indicesArray.push(Math.floor(Math.random() * total));
        }
        const indices = tf.tensor1d(indicesArray, 'int32');
        const batchXs = tf.gather(xs, indices);
        const batchYs = tf.gather(ys, indices);
        indices.dispose();
        return { xs: batchXs, ys: batchYs, indices: indicesArray };
    });
}
