...
function splitTrainVal(xs, ys, valRatio = 0.1) {
    return tf.tidy(() => {
        ...
        // Create shuffled indices as a tensor
        const indices = tf.util.createShuffledIndices(total);
        const indicesTensor = tf.tensor1d(indices, 'int32');
        
        // Split indices tensor
        const trainIndices = indicesTensor.slice([0], [trainSize]);
        const valIndices = indicesTensor.slice([trainSize], [valSize]);
...
function getRandomTestBatch(xs, ys, k = 5) {
    return tf.tidy(() => {
        ...
        // pick random indices (no replacement) and create tensor
        const indices = [];
