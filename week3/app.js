...
const CONFIG = {
  ...
  learningRate: 0.02,
  autoTrainSpeed: 80,            // ms per step
  smoothnessWeight: 0.2,        // [TODO-B] suggested starting weight
  directionWeight: 0.3,         // [TODO-B] suggested starting weight
};
...
function smoothness(yPred) {
  return tf.tidy(() => {
    // Get dimensions - ensure we don't slice out of bounds
...
function createStudentModel(archType) {
  ...
  if (archType === 'compression') {
    // Bottleneck: information is compressed (64 < 256)
    model.add(tf.layers.dense({ units: 64, activation: 'relu' }));
    model.add(tf.layers.dense({ units: 256, activation: 'sigmoid' }));
    ...
    // Goal: preserve exact capacity, rearrange pixels, no compression.
    // Using two layers of 256 units to allow complex remapping while maintaining dimension
    ...
    // Using 512 units in hidden layers for expansion
...
function resetModels(archType = null) {
  ...
  try {
    ...
    // Compile model to ensure shapes are validated
...
function init() {
  // Fixed random seed for reproducibility - using deterministic approach
  tf.setBackend('cpu');
